  ...
  return (
    <aside className={`sidebar ${collapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      <div className="sidebar-logo" style={{ display: 'flex', alignItems: 'center', padding: '16px' }}>
        ...
        <button className="sidebar-toggle-btn" onClick={onToggle} title={collapsed ? 'Expand' : 'Collapse'} style={{ marginLeft: 'auto' }}>
          {collapsed ? '☰' : '×'}
        </button>
      </div>
  ...
  return (
    <header className="top-header">
      ...
      <div className="header-right" style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <span className="live-text" style={{ marginRight: '4px' }}>Live Data</span>
        {(() => {
          const u = JSON.parse(localStorage.getItem('fintrend-current-user')) || { name: 'MAA' };
          const initials = u.name ? u.name.substring(0, 2).toUpperCase() : 'MA';
          return (
            <div 
              className="header-avatar" 
              style={{ cursor: 'pointer' }} 
              title={`Logged in as ${u.name || ''}. Click to sign out.`}
              onClick={() => {
...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState(() => {
    try {
      const saved = localStorage.getItem('fintrend-current-user');
