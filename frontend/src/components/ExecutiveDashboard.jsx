...
export default function ExecutiveDashboard() {
  ...
  const [kpis, setKpis] = useState({
    aum: { value: 2.89, changeVal: 152, changePct: 1.18, history: [2.81, 2.82, 2.84, 2.85, 2.86, 2.87, 2.89] },
    flow: { value: 324, changeVal: 142, changePct: 76.1, history: [290, 270, 250, 260, 280, 255, 324] },
    sip: { value: 14.3, changeVal: 18714, changePct: 1.31, history: [13.8, 13.9, 14.0, 14.1, 14.15, 14.2, 14.3] },
    investors: { value: 4871, changeVal: 680, changePct: 16.8, history: [4200, 4350, 4500, 4650, 4700, 4780, 4871] },
    redemptions: { value: 188, changeVal: -48, changePct: -33, history: [240, 220, 190, 210, 185, 185, 188] },
    risk: { score: 45, trend: 'Stable', history: [45, 47, 46, 45, 44, 45, 45] }
  });

  const [cashFlow, setCashFlow] = useState({ inflow: 512, outflow: 188, net: 324 });
  const [portfolio, setPortfolio] = useState({ largeCap: 21.6, midCap: 19.2, smallCap: 16.8, hybrid: 12.4, debt: 7.2 });
  
  const [schemes, setSchemes] = useState([
    { name: 'FinVista Bluechip Fund', aum: 18024, returnYtd: 21.6, benchmark: 19.8, alpha: 1.8, type: 'Equity' },
    { name: 'FinVista Midcap Fund', aum: 12845, returnYtd: 19.2, benchmark: 17.1, alpha: 2.1, type: 'Equity' },
    { name: 'FinVista Smallcap Fund', aum: 8642, returnYtd: 16.8, benchmark: 14.3, alpha: 2.5, type: 'Equity' },
    { name: 'FinVista Flexi Cap Fund', aum: 11203, returnYtd: 14.2, benchmark: 12.4, alpha: 1.8, type: 'Equity' },
    { name: 'FinVista Hybrid Equity Fund', aum: 6215, returnYtd: 12.4, benchmark: 10.9, alpha: 1.5, type: 'Hybrid' },
    { name: 'FinVista Corporate Bond Fund', aum: 7850, returnYtd: 7.8, benchmark: 7.2, alpha: 0.6, type: 'Debt' },
    { name: 'FinVista Liquid Treasury Fund', aum: 9420, returnYtd: 6.2, benchmark: 5.9, alpha: 0.3, type: 'Liquidity' }
  ]);

  const [riskCompliance, setRiskCompliance] = useState([
    { metric: 'SIP Stoppage Ratio', value: '95.46%', status: 'high', trend: 'up' },
    { metric: 'Large Redemptions Alert', value: '1', status: 'high', trend: 'up' },
    { metric: 'Compliance Exceptions', value: '2', status: 'medium', trend: 'right' },
    { metric: 'Concentration Risk', value: 'Low', status: 'low', trend: 'down' },
    { metric: 'Liquidity Risk', value: 'Low', status: 'low', trend: 'down' }
  ]);
      ...
      <div className="kpi-row-grid">
        ...
        <div className="card kpi-card">
          <div>
            ...
            <div className="kpi-value-row">
              <span className="kpi-value">₹{kpis.flow.value}</span>
        ...
        <div className="card kpi-card">
          <div>
            ...
            <div className="kpi-value-row">
              <span className="kpi-value">₹{kpis.redemptions.value}</span>
              <span className="kpi-unit">Cr</span>
            </div>
